defmodule Advent.Day do
  ...
  def enhance_extend(target, rows, cols) do
    ...
    extendx = for x <- [0,1, new_cols-2,new_cols-1] do
      for y <- 0..new_rows-1 do
        {{x, y}, inf}
      end
    end
    extendy = for x <- 0..new_cols-1 do
      for y <- [0,1,new_rows-2, new_rows-1] do
        {{x, y}, inf}
      end
    end
    # (extendx++extendy)
  ...
  def enhance(target, rows, cols, _, 0) do
    {target, rows, cols}
  end
  ...
  def exec() do
    ...
    {target, rows, cols} = enhance(target, rows, cols, algo, 50)
    count(target)
    |>log()
    :ok
  end
