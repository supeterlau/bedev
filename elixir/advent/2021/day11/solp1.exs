defmodule Advent.Day do
  ...
  def step(data, lights, 0), do: {data,lights}

  def step(data, lights, steps) do
    data = inc_data(data)
    lights = lights + (data 
             |> Enum.filter(fn {_, v} -> v==:flash end)
             |> Enum.count())
    data = data
           |> Enum.map(fn 
             {k,:flash} -> {k, 0}
  ...
  def exec() do
    ...
    IO.inspect(lights, label: "lights")
  end
end
