defmodule Advent.Day do
  ...
  def parse_input() do
     ...
     File.read!('./input.txt') |> String.split(~r{---.*---}, trim: true)
  end
  ...
  def exec() do
    ...
    # |> tranverse()
    spath = resolv_zps(zps, 0, %{}, [0], 0..length(beacons)-1)
    |> Map.values()
    [next | path] = spath
    zps = tranverse(path, next, zps, [])
    # fix_values(pbeacons|>Enum.with_index(), zps)
    # |> get_zeros( spath |> List.flatten() |> Enum.reverse(), %{})
    |> via_path()
    |> log(:via_path)
    |> max_dist()
  end

  def max_dist(zeros) do
    zeros = zeros
    |> Map.values()
    |> hd()
    |> Enum.map(fn {k, {v, _}} -> v end)
    |> List.flatten()
    zeros= [{0,0,0}|zeros]
    mdist(zeros)
  ...
  def msub(p1, p2) do
    0..tuple_size(p1)-1
    # |>Enum.map(&abs(elem(p1, &1) - elem(p2, &1)))
    |>Enum.map(&elem(p1, &1) - elem(p2, &1))
    |> List.to_tuple()
  end

  def mdist(beacons) do
    ...
    for b1 <- 0..total-2 do
      for b2 <- b1+1..total-1 do
        ...
        [
          {{p1, p2}, msub(p1, p2)|>Tuple.to_list()|>Enum.sum()},
          {{p2, p1}, msub(p2, p1)|>Tuple.to_list()|>Enum.sum()}
        ]
      end
    end
    |> List.flatten()
  ...
  def via_path(zps) do
    fkey_map = zps 
               ...
               |> Enum.into(%{})
    zps = zps
    |> Enum.reduce(zps, fn {k, v}, new_zps -> 
      if k == 0 do
        new_zps
      else
        up_key = fkey_map |> Map.get(k)
        
        {up_kv, pat} = Map.get(zps, up_key) |> Map.get(k)
        kv = if is_nil(pat) do
          v
        else
          v |> Enum.map(fn {kk, {kkv, _}} -> 
                {kk, {sub(up_kv, convert(kkv, pat)), nil}}
          end) |> Enum.into(%{})
        end
        new_zps
        |> Map.update(up_key, kv, fn old -> Map.merge(old, kv) end)
      end
    end)
    if Map.get(zps, 0) |> Map.keys() |> length() < 25 do
    # if all_zeros_nil(zps)|>log(:alll_zeros) do
      zps |> log(:zps_zps_zps)
      via_path(zps)
    else
      zps
  ...
  # spath |> List.flatten() |> Enum.reverse()
  def get_zeros(zps, [], result) do
    result
  end

  def get_zeros(zps, spath, result) do
    [next|tail] = spath
    fkey_map = zps 
               |> Enum.map(fn {k, v} ->
                 Map.keys(v) |> Enum.map(&{&1, k}) 
               ...
               |> List.flatten()
               |> Enum.into(%{})
    up_key = fkey_map |> Map.get(next)
    {up_kv, pat} = Map.get(zps, up_key) |> Map.get(next)
    kv = Map.get(zps, up_key) |> Enum.map(fn {kk, {kkv, _}} -> 
          {kk, {sub(up_kv, convert(kkv, pat)), nil}}
    end) |> Enum.into(%{})
    if tail == [0] do
      tail = Map.keys(zps)|>Enum.reverse()
      result
      get_zeros(zps, tail, result)
    else
      result = Map.update(result, up_key, kv, fn old -> Map.merge(old, kv) end)
      get_zeros(zps, tail, result)
    end
  end

  def get_zeros(zps) do
    fkey_map = zps 
               |> Enum.map(fn {k, v} ->
                 Map.keys(v) |> Enum.map(&{&1, k}) 
               end)
               ...
               |> Enum.into(%{})
    zps = zps 
    |> Enum.reduce(%{}, fn {k, kv}, new_result -> 
      if k == 0 do
        Map.put(new_result, k, kv)
      else
        up_key = fkey_map |> Map.get(k)
        {up_kv, pat} = Map.get(zps, up_key) |> Map.get(k)
        kv = kv
        |> Enum.map(fn {kk, {kkv, _}} -> 
          {kk, {sub(up_kv, convert(kkv, pat)), nil}}
        end) |> Enum.into(%{})
        Map.update(new_result, up_key, kv, fn old -> 
          Map.merge(old, kv)
        end)
      end
    end)
    if zps |> Map.keys() |> length() > 1 do
      get_zeros(zps)
    else
      zps
    end
...
# Advent.Day.msub({3690, 1094, 19}, {-3582, 1080, 1283})
# |> Advent.Day.log()
